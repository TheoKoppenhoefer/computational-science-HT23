...
params_LIF['LIF'] = 0.06
over_expressions = [0.1, 0.13, 0.2, 0.3] # [0.1,0.3,0.8]
expressions = ['N','O','T']
...
if __name__ == '__main__':
    ...
    for j, exp_label in enumerate(Experiment_labels):
        ...
        y0 = 0.7*np.ones((5 if repression else 3,1)) if exp_label[:14] == 'LIF_withdrawal' else np.zeros((5 if repression else 3,1))
        t, y = run_experiment_series(param_lists[exp_label], y0, repression)
